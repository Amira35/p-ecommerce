<template>
    <div class="qualiy-features bg-grey-lighten-4 pt-4">
        <v-container>
            <v-row>
                <v-col cols="12" sm="6" md="3"  v-for="card in cards" :key="card.title">
                    <v-card class="text-center d-flex align-center justify-center" height="250px">
                        <div class="paren">
                            <span v-html="card.svg"></span>
                            <v-card-title style="text-transform:uppercase; font-size: 14px; letter-spacing:-.5px; font-weight: 700;">{{ card.title }}</v-card-title>
                            <v-card-text style="line-height: 1.8;">{{ card.desc }}</v-card-text>
                        </div>
                    </v-card>
                </v-col>
            </v-row>
