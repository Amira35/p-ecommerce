<template>
    <div class="top-fragrances">
        <div class="top-fragrances__title">
        ...
        <swiper :pagination="{el : '.swiper-pagination' , clickable : true}"
            ...
            <swiper-slide v-for="prod in products" :key="prod.id" class="text-center">
                <v-card elevation="0">
                        ...
                        <v-card-text>
                            {{ `(${prod.title}) ${prod.description}`.length <= 72 ? `(${prod.title}) ${prod.description}` : `(${prod.title}) ${prod.description}`.substring( 0 , 72) + '...' }}
                        </v-card-text>
