<template>
    <div class="top-groceries px-5">
        <div class="top-groceries__head">
            <h2>Top Groceries</h2>
        </div>
        <v-container>
            <v-row>
        ...
        <swiper :pagination="{el : '.swiper-pagination' , clickable : true}"
            ...
            <swiper-slide v-for="prod in products" :key="prod.id" class="text-center">
                <v-card elevation="0">
                        ...
                        <v-card-text>
                            {{ `(${prod.title}) ${prod.description}`.length <= 82  ? `(${prod.title}) ${prod.description}` : `(${prod.title}) ${prod.description}`.substring( 0, 82) + '...' }}
                        </v-card-text>
